...
class DLLoader(Dataset):
    ...
    def load_annotations(self, ann_file):
        df= pd.read_csv(ann_file,usecols=['File_name','Bounding_boxes'])
        df['filename']=df.File_name.apply(_conv)
        intermed = [v for k, v in df.to_dict(orient='index').items()]
        img_infos=[]
        for x in intermed:
            box=np.array(x['Bounding_boxes'].split(", "),dtype='float32').reshape(1,4)
            val={"filename":x['filename'],
                 ...
                 'ann':{
                     'bboxes':box,
                     'labels':np.array([1]),
                     'bboxes_ignore':np.array([],dtype='float32').reshape(0,4)
                 }}
            img_infos.append(val)
        return img_infos
